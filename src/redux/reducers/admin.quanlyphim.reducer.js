...
const adminQuanLyPhimReducer = (state = initialState, action) => {
  ...
  switch (type) {
    case GET_LIST_MOVIE_SHOWING_FIL: {
      const { movieList, tenPhim } = payload;

      if (tenPhim !== "") {
        const rs = movieList?.filter(function (movie) {
          let engMovie = removeVietnameseTones(movie.tenPhim);
          let engTenPhim = removeVietnameseTones(tenPhim);
          return engMovie.toLowerCase().indexOf(engTenPhim) !== -1;
        });

        console.log(rs);

        return { movieListShowingFil: payload, movieList: rs };
      }
